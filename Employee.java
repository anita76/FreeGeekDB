...
public class Employee extends GuestUser {
    ...
    List<Pair<String, Integer>> averageAttend() throws SQLException {
        ...
        ResultSet rs = jdbcDriver.executeDataQuery("SELECT TEMP.sename, TEMP.AVG(count) FROM ("+aggregation+") AS TEMP");
        while (rs.next()){
            ans.add(new Pair<String, Integer>(rs.getString(1), rs.getInt(2)));
        }
        return ans;
    ...
    List<Pair<String,Integer>> leastPopularEvent() throws SQLException {
        List<Pair<String, Integer>> averageAttend = averageAttend();
        List<Pair<String, Integer>> out = new ArrayList<>();
        for (Pair<String, Integer> p: averageAttend){
            int all = 1;
            for(Pair<String, Integer> q: averageAttend){
                if(p.getValue()>q.getValue()) all = 0;
            }
            if(all ==1) out.add(p);
        }
